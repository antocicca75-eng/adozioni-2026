...
# ==============================================================================
    ...
    if not st.session_state.get("storico_consegne"):
        ...
        plessi_da_mostrare = st.multiselect("🔍 Seleziona Plesso/i:", elenco_plessi_storico, key="sel_plessi_storico")
        st.markdown("---")
        if not plessi_da_mostrare:
            st.info("Seleziona uno o più plessi per visualizzare il carico.")
        else:
            for plesso in plessi_da_mostrare:
                with st.expander(f"🏫 {plesso.upper()}", expanded=True):
                    with st.container(border=True):
...
# =========================================================
    ...
    if not st.session_state.storico_ritiri:
        ...
        st.markdown("---")
        if not plessi_show:
            st.info("Seleziona uno o più plessi per visualizzare i ritiri.")
        else:
