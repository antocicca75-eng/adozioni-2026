...
# =========================================================
# --- BLOCCO 2: COSTANTI E CONFIGURAZIONE PAGINA ---
# =========================================================
...
# =========================================================
# --- BLOCCO 3: CONNESSIONE GOOGLE E BACKUP ---
# =========================================================
...
# =========================================================
# --- BLOCCO 4: GESTIONE CONFIGURAZIONI E STORICO ---
# =========================================================
...
def aggiungi_libro_a_excel(t, m, e, a):
    try:
        # Nota: gspread non aggiorna Excel file fisici, questa è una funzione legacy per anagrafiche.xlsx
...
# =========================================================
# --- BLOCCO 7: STATO SESSIONE E NAVIGAZIONE ---
# =========================================================
...
if 'db_consegne' not in st.session_state: st.session_state.db_consegne = carica_config_consegne()
if 'lista_consegne_attuale' not in st.session_state: st.session_state.lista_consegne_attuale = []
...
with st.sidebar:
    ...
    st.markdown("---")
    if st.button("📊 APRI TABELLONE STATO", use_container_width=True): st.session_state.pagina = "Tabellone Stato"; st.rerun()
...
if st.session_state.pagina == "Consegne":
    ...
    p_scelto = col_p.selectbox("Seleziona Plesso:", ["- SELEZIONA PLESSO -"] + elenco_plessi)
    ...
    if st.button("✅ CONFERMA CONSEGNA", use_container_width=True):
...
# =========================================================
    ...
    if st.session_state.storico_consegne:
        scuola_selezionata = st.selectbox("🔍 Seleziona Plesso:", sorted(list(st.session_state.storico_consegne.keys())))
        if scuola_selezionata:
            for tipo, libri in st.session_state.storico_consegne[scuola_selezionata].items():
                with st.expander(f"📘 {tipo}"):
                    for lib in libri:
                        st.write(f"- {lib['t']} ({lib.get('q', 1)} copie)")

# ---------------------------------------------------------

# =========================================================
# --- BLOCCO 11: AGGIUNGI A CATALOGO ---
# =========================================================
elif st.session_state.pagina == "NuovoLibro":
    st.subheader("🆕 Aggiungi nuovo titolo")
    with st.container(border=True):
        nt = st.text_input("Titolo Libro")
        m_val = st.text_input("Materia")
        e_val = st.text_input("Editore")
        a_val = st.text_input("Agenzia")
        if st.button("✅ SALVA"):
            if nt and m_val:
                # Logica salvataggio...
...
# =========================================================
# --- BLOCCO 12: INSERIMENTO ADOZIONE ---
# =========================================================
    ...
    with st.container(border=True):
        ...
        saggio = st.selectbox("📚 Saggio consegnato", ["-", "NO", "SI"])
        if st.button("💾 SALVA ADOZIONE"):
...
# =========================================================
# --- BLOCCO 13: RICERCA E FILTRI ---
# =========================================================
elif st.session_state.pagina == "Ricerca":
    st.subheader("🔍 Motore di Ricerca Adozioni")
    with st.container(border=True):
        f_tit = st.multiselect("📕 Titolo Libro", elenco_titoli)
        f_ple = st.multiselect("🏫 Plesso", elenco_plessi)
