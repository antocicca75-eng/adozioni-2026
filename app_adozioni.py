                ...
                for tipo in sorted(list(per_tipo.keys())):
                    ...
                    with st.expander(f"Dettaglio {tipo}", expanded=True):
                        for i, lib in enumerate(list(per_tipo[tipo])):
                            ...
                            col_qta.markdown(f"<p style='text-align:center;'>In carico: <b>{qta_carico}</b></p>", unsafe_allow_html=True)
    ...
    with st.container(border=True):
        ...
        if st.button("💾 SALVA ADOZIONE", use_container_width=True, type="primary"):
            if titolo_scelto and plesso and saggio != "-":
                ...
                st.success("✅ Registrazione avvenuta con successo!"); st.rerun()
...
# =========================================================
    ...
    with st.container(border=True):
        ...
        f_tit = r1c1.multiselect("📕 Titolo Libro", elenco_titoli, key="ft")
        f_age = r1c2.multiselect("🤝 Agenzia", elenco_agenzie, key="fa")
        f_sag = r1c3.selectbox("📚 Saggio consegnato", ["TUTTI", "SI", "NO"], key="fsag")
        r2c1, r2c2, r2c3 = st.columns(3)
        f_ple = r2c1.multiselect("🏫 Plesso", elenco_plessi, key="fp")
        f_mat = r2c2.multiselect("📖 Materia", elenco_materie, key="fm")
        f_edi = r2c3.multiselect("🏢 Editore", elenco_editori, key="fe")
        if st.button("🔍 AVVIA RICERCA", type="primary"): st.session_state.r_attiva = True
        if st.button("🧹 PULISCI"): reset_ricerca(); st.rerun()
