...
def get_catalogo_libri():
    ...
    if os.path.exists(CONFIG_PATH):
        try:
            df = pd.read_excel(CONFIG_PATH, sheet_name="ListaLibri")
...
def get_lista_plessi():
    ...
    if os.path.exists(CONFIG_PATH):
        try:
            df = pd.read_excel(CONFIG_PATH, sheet_name="Plesso")
            return sorted(df.iloc[:, 0].dropna().unique().tolist())
        except: return []
    return []
...
if st.session_state.pagina == "NuovoLibro":
    ...
    with st.container(border=True):
        ...
        if st.button("💾 SALVA ADOZIONE", use_container_width=True, type="primary"):
            if titolo_scelto and plesso and saggio != "-":
                ...
                df_attuale = pd.read_csv(DB_PATH) if os.path.exists(DB_PATH) else pd.DataFrame()
    ...
    if os.path.exists(DB_PATH):
        df_mod = pd.read_csv(DB_PATH).fillna("").astype(str)
                    ...
                    with st.container(border=True):
                        ...
                        with b1:
                            if st.button("💾 AGGIORNA", key=f"upd_{i}", use_container_width=True, type="primary"):
                                if nuovo_saggio != "-":
                                    df_full = pd.read_csv(DB_PATH).fillna("").astype(str)
                                    ...
                                    df_full.to_csv(DB_PATH, index=False); backup_su_google_sheets(df_full)
                        ...
                        with b2:
                            if st.button("🗑️ ELIMINA", key=f"del_{i}", use_container_width=True):
                                df_full = pd.read_csv(DB_PATH).fillna("").astype(str); df_full = df_full.drop(int(i))
                                df_full.to_csv(DB_PATH, index=False); backup_su_google_sheets(df_full); st.rerun()

elif st.session_state.pagina == "Registro":
    st.subheader("📑 Registro Completo")
    if os.path.exists(DB_PATH):
        st.dataframe(pd.read_csv(DB_PATH).sort_index(ascending=False), use_container_width=True)
    ...
    if st.session_state.r_attiva and os.path.exists(DB_PATH):
        df = pd.read_csv(DB_PATH).fillna("").astype(str)
